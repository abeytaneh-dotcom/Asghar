...
static void sendDiag() {
  ...
  bleOut("D1|FW" + String(FW_VERSION) + "|S" + String(simMode ? 1 : 0));
  delay(8);
...
static void handleBleCommand(String s) {
  ...
  if (s == "STATUS") {
    bleOut(simMode ? "ST|SIM|1" : "ST|NONE|0");
    return;
  }
  if (s == "D?") {
    sendDiag();
  ...
  // Real end-to-end bridge commands:
  // Web App -> BLE -> ESP -> USB -> Simulator -> USB -> ESP -> BLE -> Web App
  if (s.startsWith("W|P|") || s.startsWith("W|A|") || s == "W|S" ||
