 ...
 class ClusterView extends View{
  ...
  protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight();boolean land=w>h;c.drawColor(BG);
   if(!land){
    ...
    float gy=h*.205f,rad=Math.min(w*.245f,h*.13f);gauge(c,w*.25f,gy,rad,Math.min(1,rpm/8000f),true);gauge(c,w*.75f,gy,rad,Math.min(1,speed/240f),false);
    float roadTop=h*.335f,roadBot=h*.555f;city(c,0,roadTop,w,roadBot);car(c,w/2,roadTop+h*.035f,Math.max(1.55f,w/430f));
    p.setStyle(Paint.Style.FILL);p.setColor(Color.argb(120,0,25,48));Path rd=new Path();rd.moveTo(w*.18f,roadBot);rd.lineTo(w*.42f,roadTop+h*.11f);rd.lineTo(w*.58f,roadTop+h*.11f);rd.lineTo(w*.82f,roadBot);rd.close();c.drawPath(rd,p);glowLine(c,w*.49f,roadTop+h*.13f,w*.45f,roadBot,CYAN,2);glowLine(c,w*.51f,roadTop+h*.13f,w*.55f,roadBot,CYAN,2);
    float sy=h*.575f,g=7,cw=(w-28)/3f,ch=h*.105f;stat(c,7,sy,7+cw,sy+ch,"T","دمای آب",temp+" °C",RED);stat(c,14+cw,sy,14+2*cw,sy+ch,"F","سوخت","48 %",AMBER);stat(c,21+2*cw,sy,w-7,sy+ch,"V","ولتاژ","13.8 V",GREEN);
    float ey=sy+ch+8;rr(c,7,ey,w-7,ey+h*.072f,14,Color.rgb(4,25,37),Color.rgb(20,90,105));tx(c,"✓  وضعیت موتور: سالم",20,ey+h*.030f,14,GREEN,Paint.Align.LEFT,true);tx(c,"ECU: CAN 500kbps",w-18,ey+h*.030f,12,Color.WHITE,Paint.Align.RIGHT,true);tx(c,"خودرو: TU5 • OBD-II • ارتباط پایدار",w-18,ey+h*.055f,11,MUTED,Paint.Align.RIGHT,false);
    float ty=ey+h*.083f,th=h*.095f,tw=(w-30)/4f;tile(c,6,ty,6+tw,ty+th,"ECU","دیاگ",BLUE);tile(c,12+tw,ty,12+2*tw,ty+th,"▥","داده زنده",AMBER);tile(c,18+2*tw,ty,18+3*tw,ty+th,"!","کد خطا",RED);tile(c,24+3*tw,ty,w-6,ty+th,"✓","تست‌ها",Color.rgb(160,80,255));
    float ny=h-h*.085f;rr(c,0,ny,w,h,0,Color.rgb(2,12,22),0);glowLine(c,0,ny,w,ny,Color.rgb(0,80,125),1);String[] n={"⌂\\nخانه","▣\\nخودرو","▥\\nگزارش","⚙\\nتنظیمات"};for(int i=0;i<4;i++){float cx=w*(i+.5f)/4;tx(c,n[i].split("\\\\n")[0],cx,ny+h*.032f,24,i==0?CYAN:MUTED,Paint.Align.CENTER,true);tx(c,n[i].split("\\\\n")[1],cx,ny+h*.063f,12,i==0?Color.WHITE:MUTED,Paint.Align.CENTER,false);}
   }else{
