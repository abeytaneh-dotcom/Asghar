...
public class MainActivity extends Activity {
 ...
 class ClusterView extends View{
  ...
  protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight();boolean land=w>h;c.drawColor(BG);
   if(!land){
    float header=h*.055f;rr(c,8,7,w-8,header,16,Color.rgb(3,18,31),Color.rgb(12,65,98));tx(c,"☰",26,header*.67f,24,Color.WHITE,Paint.Align.CENTER,true);tx(c,"خانه ریمپ",w*.68f,header*.43f,20,Color.WHITE,Paint.Align.CENTER,true);tx(c,"SMART OBD",w*.68f,header*.76f,14,CYAN,Paint.Align.CENTER,true);rr(c,w-145,12,w-14,header-6,12,Color.rgb(4,27,44),CYAN);tx(c,"● ESP32_OBD",w-80,header*.42f,11,Color.WHITE,Paint.Align.CENTER,true);tx(c,demo?"DEMO":"LIVE",w-80,header*.72f,10,demo?AMBER:GREEN,Paint.Align.CENTER,true);
    float clusterTop=header+10,clusterBottom=h*.38f,gy=(clusterTop+clusterBottom)*.51f,rad=Math.min(w*.235f,(clusterBottom-clusterTop)*.48f);gauge(c,w*.245f,gy,rad,Math.min(1,rpm/8000f),true);gauge(c,w*.755f,gy,rad,Math.min(1,speed/240f),false);
    float sceneTop=h*.39f,sceneBot=h*.585f;city(c,0,sceneTop,w,sceneBot);p.setStyle(Paint.Style.FILL);LinearGradient sky=new LinearGradient(0,sceneTop,0,sceneBot,Color.rgb(2,13,28),Color.rgb(1,7,14),Shader.TileMode.CLAMP);p.setShader(sky);c.drawRect(0,sceneTop,w,sceneBot,p);p.setShader(null);
    Path road=new Path();road.moveTo(w*.08f,sceneBot);road.lineTo(w*.43f,sceneTop+h*.065f);road.lineTo(w*.57f,sceneTop+h*.065f);road.lineTo(w*.92f,sceneBot);road.close();p.setColor(Color.rgb(5,18,29));c.drawPath(road,p);glowLine(c,w*.46f,sceneBot,w*.492f,sceneTop+h*.09f,CYAN,2);glowLine(c,w*.54f,sceneBot,w*.508f,sceneTop+h*.09f,CYAN,2);car(c,w/2,sceneTop+h*.025f,Math.max(1.8f,w/360f));
    float sy=h*.595f,cw=(w-35)/4f,ch=h*.105f;stat(c,7,sy,7+cw,sy+ch,"T","دمای آب",temp+" °C",RED);stat(c,14+cw,sy,14+2*cw,sy+ch,"F","سوخت","48 %",AMBER);stat(c,21+2*cw,sy,21+3*cw,sy+ch,"V","باتری","13.8 V",GREEN);stat(c,28+3*cw,sy,w-7,sy+ch,"P","سطح آب","مناسب",GREEN);
    float ey=sy+ch+8;rr(c,7,ey,w-7,ey+h*.058f,14,Color.rgb(4,25,37),Color.rgb(24,105,94));tx(c,"● موتور سالم",20,ey+h*.035f,13,GREEN,Paint.Align.LEFT,true);tx(c,"CAN 500 kbps  •  OBD-II  •  TU5",w-18,ey+h*.035f,11,Color.WHITE,Paint.Align.RIGHT,true);
    float ty=ey+h*.071f,th=h*.083f,tw=(w-30)/4f;tile(c,6,ty,6+tw,ty+th,"ECU","دیاگ",BLUE);tile(c,12+tw,ty,12+2*tw,ty+th,"▥","داده زنده",AMBER);tile(c,18+2*tw,ty,18+3*tw,ty+th,"!","DTC",RED);tile(c,24+3*tw,ty,w-6,ty+th,"✓","تست عملگر",Color.rgb(160,80,255));
